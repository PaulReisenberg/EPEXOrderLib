...
private:
    ...
    static ExecutionRestriction stringToExecutionRestriction(const std::string& s) {
        ...
        if (s == "FOK") return ExecutionRestriction::FOK;
        if (s == "IOC") return ExecutionRestriction::IOC;
...
public:
    ...
    std::string toString() const {
        ...
        oss << std::string(idWidth * 3 + sideWidth + timeWidth * 2 + areaWidth + 
                        restrictionWidth + blockWidth + revWidth + actionWidth + 
                        priceWidth + volumeWidth, '-') << "\n";
        ...
        for (const auto& row : rows) {
            oss << std::left
                ...
                << std::setw(revWidth) << row.revisionNo
...
private:
    ...
    static std::string executionRestrictionToString(ExecutionRestriction er) {
        switch (er) {
            ...
            case ExecutionRestriction::FOK: return "FOK";
            case ExecutionRestriction::IOC: return "IOC";
            default: return "UNKNOWN";
        }
    }
...
EventData getLOB(EventData eventData, std::chrono::system_clock::time_point time_stamp) {
    ...
    auto leq_time_stamp = [time_stamp](const EventDataRow& row){return row.transactionTime <= time_stamp;};
    ...
    auto get_max_revNo = [&maxRevMap](const EventDataRow& row){
        if(maxRevMap.find(row.orderId) == maxRevMap.end() || maxRevMap[row.orderId] < row.revisionNo) {
    ...
    auto delete_not_max = [&maxRevMap](const EventDataRow& row){return row.revisionNo == maxRevMap[row.orderId];};
